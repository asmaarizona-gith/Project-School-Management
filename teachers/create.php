...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $check = $db->prepare("SELECT id FROM teachers WHERE email = :email");
        $check->execute([':email' => $form_data['email']]);
        if ($check->fetch()) {
    ...
    if (empty($errors)) {
        $stmt = $db->prepare("INSERT INTO teachers (firstname, lastname, email, phone, address, gender, qualification, subject_id, class_id) VALUES (:firstname, :lastname, :email, :phone, :address, :gender, :qualification, :subject_id, :class_id)");
        $stmt->execute([
        ...
        $_SESSION['flash'] = ['type' => 'success', 'message' => 'Teacher added successfully!'];
...
<div class="main-content">
    <div class="content-header">
        <h4 class="fw-bold"><i class="fas fa-chalkboard-teacher me-2"></i>Add New Teacher</h4>
