...
$class = $db->prepare("SELECT * FROM classes WHERE id = :id");
$class->execute([':id' => $id]);
$class = $class->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $db->prepare("UPDATE classes SET name = :name, section = :section, room = :room, capacity = :capacity WHERE id = :id");
        $stmt->execute([
            ':name' => $form_data['name'],
        ...
        $_SESSION['flash'] = ['type' => 'success', 'message' => 'Class updated successfully!'];
        header('Location: index.php');
        exit;
    }
}
...
<div class="main-content">
    <div class="content-header">
        <h4 class="fw-bold"><i class="fas fa-edit me-2"></i>Edit Class</h4>
