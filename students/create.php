...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $check = $db->prepare("SELECT id FROM students WHERE email = :email OR admission_no = :admission_no");
        $check->execute([':email' => $form_data['email'], ':admission_no' => $form_data['admission_no']]);
        if ($check->fetch()) {
    ...
    if (empty($errors)) {
        $stmt = $db->prepare("INSERT INTO students (firstname, lastname, email, phone, address, gender, date_of_birth, admission_no, class_id) VALUES (:firstname, :lastname, :email, :phone, :address, :gender, :date_of_birth, :admission_no, :class_id)");
        $stmt->execute([
...
<div class="main-content">
    <div class="content-header">
        <h4 class="fw-bold"><i class="fas fa-user-graduate me-2"></i>Add New Student</h4>
